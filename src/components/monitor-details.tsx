...
export function MonitorDetails({ id: key }: Props) {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:${port}/_/servers`)
      ...
      .then((response) => {
        ...
        setConfig({
          ...
          env: filterObject(
            server.env,
            (key) => !sameEnvVars.includes(key) && key !== "PATH"
          ),
        });
      })
      .catch((err) =>
...
const findSameProperties = (items: Record<string, string>[]) =>
  Object.keys(
    items.reduce((prev, curr) =>
      entriesToObject(
        Object.entries(prev).filter(
          ([key, value]) => key in curr && curr[key] === value
        )
      )
    )
  );
  ...
  return (
    <Detail.Metadata>
      <Detail.Metadata.Label
        title="command"
        text={config.command?.reverse()[0]}
      />
      <Detail.Metadata.Label
        title="cwd"
        text={config.cwd?.replace(homedir(), "~")}
      />
