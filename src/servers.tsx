...
export default function Command() {
  ...
  const [filteredMonitors, setFilteredMonitors] = useState<Record<string, Monitor>>({});
  const [searchText, setSearchText] = useState("");

  useEffect(() => {
    setFilteredMonitors(searchText?.trim()
  ...
  useEffect(
    () => console.log(Object.entries(monitors).map(([key, {status}]) => `${key}: ${status}`)),
    [monitors],
  )
